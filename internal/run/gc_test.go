...
func TestPruneRunArtifactsRetainsCutoffSecond(t *testing.T) {
	t.Parallel()
	now := time.Unix(10_000, 750_000_000)
	maxAge := time.Hour
	store := &retentionCountingStore{
		objects: []blob.ListedObject{{
			Key:          "runs/cutoff-second",
			Version:      "v1",
			LastModified: now.Add(-maxAge).Unix(),
		}},
		deleteErr: map[string]error{},
	}

	deleted, err := PruneRunArtifacts(t.Context(), store, maxAge, now)
	if err != nil {
		t.Fatal(err)
	}
	if deleted != 0 || len(store.deleteCalls) != 0 {
		t.Fatalf("cutoff-second object was deleted: deleted=%d calls=%v", deleted, store.deleteCalls)
	}
}

func TestPruneRunArtifactsReportsUnsupportedConditionalDelete(t *testing.T) {
	t.Parallel()
	store := &retentionCountingStore{
		objects: []blob.ListedObject{
			{Key: "runs/first", Version: "v1", LastModified: 1},
			{Key: "runs/second", Version: "v2", LastModified: 1},
		},
		deleteErr: map[string]error{
			"runs/first": blob.ErrConditionalDeletesUnsupported,
		},
	}

	deleted, err := PruneRunArtifacts(t.Context(), store, time.Hour, time.Unix(10_000, 0))
	if !errors.Is(err, blob.ErrConditionalDeletesUnsupported) {
		t.Fatalf("prune error = %v, want ErrConditionalDeletesUnsupported", err)
	}
	if deleted != 0 || len(store.deleteCalls) != 1 {
		t.Fatalf("prune continued after unsupported delete: deleted=%d calls=%v", deleted, store.deleteCalls)
	}
}

func TestPruneRunArtifactsReportsConditionalDeleteProbeFailure(t *testing.T) {
	t.Parallel()
	store := &retentionCountingStore{
		objects: []blob.ListedObject{
			{Key: "runs/first", Version: "v1", LastModified: 1},
		...
		deleteErr: map[string]error{
			"runs/first": blob.ErrConditionalDeleteProbeFailed,
		},
	}

	deleted, err := PruneRunArtifacts(t.Context(), store, time.Hour, time.Unix(10_000, 0))
	if !errors.Is(err, blob.ErrConditionalDeleteProbeFailed) {
		t.Fatalf("prune error = %v, want ErrConditionalDeleteProbeFailed", err)
	}
	if deleted != 0 || len(store.deleteCalls) != 1 {
		t.Fatalf("prune continued after probe failure: deleted=%d calls=%v", deleted, store.deleteCalls)
	}
}
