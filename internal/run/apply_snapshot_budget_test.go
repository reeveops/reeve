...
func TestApplyReadsLargePreviewHistoryOnce(t *testing.T) {
	...
	// plainApplyInput writes one manifest. Add 98 historical candidates and
	// one authoritative manifest for a total history of 100 objects.
	for i := 0; i < 98; i++ {
	...
	if totalReads != 100 {
		t.Fatalf("manifest content reads = %d, want 100", totalReads)
	}
}
