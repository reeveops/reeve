...
type Subject struct {
	...
	ExpectConditionalDeletesUnsupported bool
}
...
func RunContract(t *testing.T, subject Subject) {
	...
	t.Run("ListMetadata", func(t *testing.T) { testListMetadata(t, subject) })
	t.Run("ConditionalDelete", func(t *testing.T) { testConditionalDelete(t, subject) })
...
func testListMetadata(t *testing.T, subject Subject) {
	store := newMaintenanceStore(t, subject)
	const key = "runs/pr-1/run-1/manifest.json"
	put(t, store, key, "manifest")
	objects, err := store.ListMetadata(t.Context(), key)
	if err != nil {
		t.Fatalf("ListMetadata: %v", err)
	}
	if len(objects) != 1 {
		t.Fatalf("ListMetadata returned %d objects, want 1", len(objects))
	}
	got := objects[0]
	if got.Key != key || got.Version == "" || got.LastModified == 0 || got.Size != int64(len("manifest")) {
		t.Fatalf("ListMetadata object = %+v", got)
	}
}

func testConditionalDelete(t *testing.T, subject Subject) {
	store := newMaintenanceStore(t, subject)
	const key = "runs/pr-1/run-1/manifest.json"
	put(t, store, key, "old")
	objects, err := store.ListMetadata(t.Context(), key)
	if err != nil || len(objects) != 1 {
		t.Fatalf("initial ListMetadata: objects=%v err=%v", objects, err)
	}
	staleVersion := objects[0].Version
	put(t, store, key, "new")
	deleteErr := store.DeleteIfMatch(t.Context(), key, staleVersion)
	if subject.ExpectConditionalDeletesUnsupported {
		if !errors.Is(deleteErr, blob.ErrConditionalDeletesUnsupported) {
			t.Fatalf("stale DeleteIfMatch = %v, want declared ErrConditionalDeletesUnsupported", deleteErr)
		}
	} else if !errors.Is(deleteErr, blob.ErrPreconditionFailed) {
		t.Fatalf("stale DeleteIfMatch = %v, want ErrPreconditionFailed", deleteErr)
	}
	if got, _ := get(t, store, key); got != "new" {
		t.Fatalf("stale DeleteIfMatch left %q, want %q", got, "new")
	}
	if subject.ExpectConditionalDeletesUnsupported {
		return
	}
	objects, err = store.ListMetadata(t.Context(), key)
	if err != nil || len(objects) != 1 {
		t.Fatalf("current ListMetadata: objects=%v err=%v", objects, err)
	}
	if err := store.DeleteIfMatch(t.Context(), key, objects[0].Version); err != nil {
		t.Fatalf("current DeleteIfMatch: %v", err)
	}
	if _, _, err := store.Get(t.Context(), key); !errors.Is(err, blob.ErrNotFound) {
		t.Fatalf("Get after DeleteIfMatch = %v, want ErrNotFound", err)
	}
}
...
func newMaintenanceStore(t *testing.T, subject Subject) maintenanceStore {
	t.Helper()
	store := newStore(t, subject)
	maintenance, ok := store.(maintenanceStore)
	if !ok {
		t.Fatalf("blob store %T does not implement maintenance operations", store)
	}
	return maintenance
}
